...
export default function DashboardPage() {
  ...
  return (
    <div className="min-h-screen bg-background">
                ...
                <div className="space-y-4">
                  {rules.map((rule) => (
                    <div key={rule.id} className="flex items-center justify-between rounded-lg border p-4">
                      <div>
                        <p className="font-semibold">{rule.days_of_week.split(',').join(', ')}</p>
                       ...
                       <div className="flex items-center gap-2">
                        ...
                        <Button variant="ghost" size="icon" onClick={() => openDeleteDialog(rule.id)} className="text-destructive hover:text-destructive">
      ...
      <AlertDialog open={dialogState.isOpen} onOpenChange={(open) => !open && setDialogState({ isOpen: false, ruleId: null })}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Are you sure?</AlertDialogTitle>
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the availability rule.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
