...
import { AvailabilityRulePayload, PaginatedData, User } from "../types";
...
export const getHosts = async (specialty?: string): Promise<PaginatedData<Host>> => {
  const params = new URLSearchParams();
  if (specialty) {
    params.append("specialty", specialty);
  }

  const response = await api.get<PaginatedData<Host>>(`/hosts?${params.toString()}`);
  return response.data;
}
