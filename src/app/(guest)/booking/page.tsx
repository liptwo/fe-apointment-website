...
export default function BookingPage() {
  ...
  const fetchInitialData = useCallback(async () => {
    ...
    try {
      ...
      const uniqueDates = [
        ...new Set(timeslotsData.map((ts: TimeSlot) => ts.date))
      ].filter(Boolean) as string[]

      if (uniqueDates.length > 0) {
        setSelectedDate(parseISO(uniqueDates[0]))
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // setSuccess(true) // Demo - REMOVED fake success
      // Show error to user (you might want to add an error state)
      alert('Đặt lịch thất bại: ' + (error instanceof Error ? error.message : 'Lỗi không xác định'))
    } finally {
      setSubmitting(false)
    }
  }
